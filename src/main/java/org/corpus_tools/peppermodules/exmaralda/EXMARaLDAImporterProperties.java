...
public class EXMARaLDAImporterProperties extends PepperModuleProperties {
  ...
  /**
   * When this property is true, all tokens defined in exmaralda are trimmed before they are mapped
   * to Salt. That means for instance trailing blanks are removed.
   **/
  ...
  public static final String PROP_STRICT_FILE_TYPE = "strictFileType";


  public EXMARaLDAImporterProperties() {
    this.addProperty(new PepperModuleProperty(PROP_TOKEN_TIER, String.class,
        "With this property you can mark the Tier object which shall be interpreted as the one containing the tokenization and the primary text.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_TOKENSEP, String.class,
        "With this property you can set a single sign or a sequence of signs which shall be used between the concatenation of event values representing the primary data.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_TIERMERGE, String.class,
        "In EXMARaLDA there is just one value per each Event object. When mapping those data to Salt, this restriction results, that a SSpan or SToken object will be created for each Event object. To avoid this, you can mark a sequence of sets of tiers to map them to a single SSpan or SToken object in Salt.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_LAYERS_SMALL, String.class,
        "With this property you can use the layer mechanism of Salt. This means, with this property, you can map all SToken or SSpan object caused by Event objects to an SLayer object. You can also group SToken or SSpan coming from Event objects contained in several Tier objects to the same SLayer object.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_LAYERS_BIG, String.class,
        "With this property you can use the layer mechanism of Salt. This means, with this property, you can map all SToken or SSpan object caused by Event objects to an SLayer object. You can also group SToken or SSpan coming from Event objects contained in several Tier objects to the same SLayer object.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_URI_ANNOTATION, String.class,
        ...
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_SALT_SEMANTICS_POS, String.class,
        "You can influence the creation of objects in Salt to have a more semantic typing when mapping data to Salt. Here we provide three properties which can be used for a closer definition or typing of SAnnotation, SToken or SSpan objects conform to ISOCat1. This can be important in the case of a further processing with Pepper. Some modules exist, which only can deal with semantical enriched data for example they need a special kind of annotation like part-of-speech for their processing.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_SALT_SEMANTICS_LEMMA, String.class,
        "You can influence the creation of objects in Salt to have a more semantic typing when mapping data to Salt. Here we provide three properties which can be used for a closer definition or typing of SAnnotation, SToken or SSpan objects conform to ISOCat1. This can be important in the case of a further processing with Pepper. Some modules exist, which only can deal with semantical enriched data for example they need a special kind of annotation like part-of-speech for their processing.",
        false));
    this.addProperty(new PepperModuleProperty<String>(PROP_SALT_SEMANTICS_WORD, String.class,
        "You can influence the creation of objects in Salt to have a more semantic typing when mapping data to Salt. Here we provide three properties which can be used for a closer definition or typing of SAnnotation, SToken or SSpan objects conform to ISOCat1. This can be important in the case of a further processing with Pepper. Some modules exist, which only can deal with semantical enriched data for example they need a special kind of annotation like part-of-speech for their processing.",
        false));
    this.addProperty(
        new PepperModuleProperty<Boolean>(PROP_CLEAN_MODEL, Boolean.class, "....", false, false));
    this.addProperty(new PepperModuleProperty<Boolean>(PROP_TRIM_EVENTS, Boolean.class,
        "When this property is true, all events in exmaralda are trimmed before they are mapped to Salt. That means for instance trailing blanks are removed. ",
        false, false));
    this.addProperty(new PepperModuleProperty<Boolean>(PROP_PARSE_NAMESPACE, Boolean.class,
        "Parse the qualified annotation name including the namespace (e.g. myns::annoname) from the category name.",
        false, false));
    this.addProperty(new PepperModuleProperty<Boolean>(PROP_MAP_TIMELINE, Boolean.class,
        "Import the EXMARaLDA timeline as Salt STimeline", true, false));
    this.addProperty(PepperModuleProperty.create().withName(PROP_MAP_DESCRIPTIONS)
        .withType(Boolean.class).withDescription("Map tiers of the type \\\"Description\\\"")
        .withDefaultValue(true).build());
    this.addProperty(
        PepperModuleProperty.create().withName(PROP_MAP_SPEAKER_METADATA).withType(Boolean.class)
            .withDescription("Import the metadata of the speaker").withDefaultValue(true).build());
    this.addProperty(PepperModuleProperty.create().withName(PROP_TEXT_NAME_WITH_NAMESPACE)
        .withType(Boolean.class)
        .withDescription(
            "If true, texts will get qualified names with speaker name as namespace. If false, no namespace is used.")
        .withDefaultValue(false).build());

    this.addProperty(
        PepperModuleProperty.create().withName(PROP_STRICT_FILE_TYPE).withType(Boolean.class)
            .withDescription("Restrict importing files to having the file ending '.exb'.")
            .withDefaultValue(true).build());
  }
   ...
   * String for regex for for tier to layer mapping
   */
  private static final String SIMPLE_TIER_LIST_DESC =
      "\\{" + TIER_NAME_DESC + "(,\\s?" + TIER_NAME_DESC + ")*" + "\\}";
  /**
   * String for regex for for tier to layer mapping
   */
   ...
   * String for regex for for tier to layer mapping
   */
  private static final String SIMPLE_LAYER_DESC =
      "\\{" + LAYER_NAME_DESC + SIMPLE_TIER_LIST_DESC + "\\}";

  /**
   * Checks the given properties, if the necessary ones are given and creates the data-structures
   * being needed to store the properties. Throws an exception, if the needed properties are not
   * there.
   * 
   * @return Relates the name of the tiers to the layers, to which they shall be append.
   */
  public Map<String, SLayer> getTier2SLayers() {
    ...
    if ((tier2SLayerStr != null) && (!tier2SLayerStr.trim().isEmpty())) {
      ...
      if (numberOfClosingBrackets != numberOfOpeningBrackets) {
        throw new PepperModuleException(
            "Cannot import the given data, because property file contains a corrupt value for property '"
                + EXMARaLDAImporterProperties.PROP_LAYERS_BIG
                + "'. Please check the breckets you used.");
      }
      ...
      while (matcher.find()) {
        ...
        String sLayerName =
            matcher.group().replace(tierNameList, "").replace("}", "").replace("{", "");
      ...
      if (tierNames2SLayers.size() == 0) {
        logger.warn(
            "[EXMARaLDAImporter] It seems as if there is a syntax failure in the given special-param file in property '"
                + EXMARaLDAImporterProperties.PROP_LAYERS_BIG
                + "'. A value is given, but the layers to named could not have been extracted.");
      }
    }
  ...
  public boolean getStrictFileType() {
    return (Boolean) getProperty(PROP_STRICT_FILE_TYPE).getValue();
  }
}
